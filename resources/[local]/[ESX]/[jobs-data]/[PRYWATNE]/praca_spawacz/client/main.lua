...
local index = GetRandomIntInRange(1, #Config.Spawanie)

for k, v in pairs(Config.Zones) do
  if v.Pos.x == Config.Spawanie[index].x and v.Pos.y == Config.Spawanie[index].y and v.Pos.z == Config.Spawanie[index].z then
    return k
  end
end
end
...
if cancel then
  ...
  TriggerServerEvent('esx_spawacz:GiveItem')
  StartNPCJob()
  Done = true
end
end
...
AddEventHandler('esx_spawacz:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_spawacz:hasExitedMarker', function(zone)

if zone == 'Vente' then
TriggerServerEvent('esx_spawacz:stopVente')
end
CurrentAction = nil
ESX.UI.Menu.CloseAll()
end)
...
TriggerEvent('esx_spawacz:hasEnteredMarker', currentZone)
end
if not isInMarker and HasAlreadyEnteredMarker then
HasAlreadyEnteredMarker = false
TriggerEvent('esx_spawacz:hasExitedMarker', LastZone)
end
end
end
end)
...
TriggerServerEvent('esx_spawacz:startVente')
end
...
Citizen.CreateThread(function()
  ...
  RequestModel(GetHashKey("s_m_y_pestcont_01"))
  while not HasModelLoaded(GetHashKey("s_m_y_pestcont_01")) do
    Wait(1)
  end

  local ped =  CreatePed(1, 0x48114518, -331.02, -2786.0, 4.14, 82.82, false, true)
    SetEntityHeading(ped, 82.82)
