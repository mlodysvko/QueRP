...
local function HarvestMorf(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		if PlayersHarvestingMorf[source] then
			...
			local morf = xPlayer.getInventoryItem('coke')
...
local function TransformMorf(source)
	...
	SetTimeout(Config.TimeToProcess, function()
		if PlayersTransformingMorf[source] then
			local xPlayer = ESX.GetPlayerFromId(source)
			local morfQuantity = xPlayer.getInventoryItem('coke').count
...
ESX.RegisterUsableItem('coke_pooch', function(source)
	...
	sendToDiscord (('Uzyto coke!'), "Gracz " .. identifier .. " " .. " uzyl coke licka gracza: " .. xPlayer.identifier .. " i otrzymal 40% armora") 
end)